...
def etri_read_xyz_length(file, max_body=4, num_joint=25):  # 取了前两个body
    ...
    frame_index_list = np.sort(np.unique(x['frameNum'].tolist()))
    numFrame = len(frame_index_list)
    numFrame = min(numFrame, 300)
    return numFrame
...
def ff_length(args):
    ...
    data = etri_read_xyz_length(input_name, max_body=max_body_kinect, num_joint=num_joint)
    np.save(save_each_name, data)
    return None



def etri_gendata_length_multiprocessing(data_path, out_path, ignored_sample_path, 
    ...
    with multiprocessing.Pool() as pool:
        ...
        with tqdm(total=len(iterators)) as pbar:
            for _ in pool.imap_unordered(ff_length, iterators):
                pbar.update()

        # pool.map(ff_length, iterators)

    original_base_path="/home/yqs/liuhc/action/data/etri220429"
    ...
    for i, s in enumerate(tqdm(sample_name)):
        ...
        n_frames = np.load(tmp_file_name).item()
...
if __name__ == '__main__':
    ...
    arg.out_folder = "/home/yqs/liuhc/action/data/etri220501_resample64"
    ...
    for b in benchmark:
        for p in part:
            ...
            etri_gendata_length_multiprocessing(
                arg.data_path,
