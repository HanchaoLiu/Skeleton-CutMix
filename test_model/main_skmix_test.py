...
def get_parser():
    ...
    parser.add_argument('--weight_aug',type=float,default=0.8,help='weight decay for optimizer')
    parser.add_argument('--weight_dst_noAug',type=float,default=1.0,help='weight decay for optimizer')

    parser.add_argument('--output_name',type=str,default='',help='weight decay for optimizer')
...
class Processor():
    ...
    def print_log(self, str, print_time=True):
        ...
        if self.arg.print_log:
            with open('{}/log_test.txt'.format(self.arg.work_dir), 'a') as f:
                print(str, file=f)
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data_src, data_dst, data_aug, data_aug_2, label, index) in enumerate(process):
            self.global_step += 1
            # get data
            ...
            if self.arg.use_aug:
                data_aug = Variable(data_aug.float().cuda(self.output_device), requires_grad=False)
                data_aug_2 = Variable(data_aug_2.float().cuda(self.output_device), requires_grad=False)

            label = Variable(label.long().cuda(self.output_device), requires_grad=False)
            timer['dataloader'] += self.split_time()

            weight_aug = self.arg.weight_aug
            weight_dst = 1 - weight_aug
            ...
            if weight_dst!=0:
                output_dst = self.model(data_dst)
                loss_dst = self.loss(output_dst, label)
            else:
                loss_dst = torch.tensor(0.0).cuda()

            if self.arg.use_aug:
                if weight_aug !=0:
                    output_aug = self.model(data_aug)
                    loss_aug = self.loss(output_aug, label)
                else:
                    loss_aug = torch.tensor(0.0).cuda()
            else:
                loss_aug = torch.tensor(0.0).cuda()
            ...
            if self.arg.use_aug:
                loss = loss_src + loss_dst * weight_dst + loss_aug * weight_aug
            else:
                loss = loss_src + loss_dst * self.arg.weight_dst_noAug
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        for ln in loader_name:
            ...
            for batch_idx, (data, label, index) in enumerate(process):
                ...
                pred_label_list += list(predict_label.cpu().numpy())
                gt_label_list += list(label.data.cpu().numpy())
            ...
            save_pred_name = os.path.join(self.arg.work_dir, self.arg.output_name+".npy")
            print("save to ", save_pred_name)
            pred_label_list = np.array(pred_label_list).reshape((-1,1)).astype(int)
            gt_label_list = np.array(gt_label_list).reshape((-1,1)).astype(int)
            x = np.concatenate([pred_label_list, gt_label_list], 1)
            print(x.shape)
            np.save(save_pred_name, x)
    ...
    def start(self):
        ...
        # remove old log dir
        self.print_file_name = '{}/log_test.txt'.format(self.arg.work_dir)
        ...
        if self.arg.phase == 'train':
            ...
            old_log_file = '{}/log_test.txt'.format(self.arg.work_dir)
            new_log_file = '{}/log_test.txt'.format(self.arg.base_dir)
